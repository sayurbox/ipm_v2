...
def reorder_point_reached(sku):
    # Load current inventory level
    inventory_manager = InventoryManager()
    current_inventory = inventory_manager.get_inventory(sku)

    # Calculate next delivery date and next to next delivery date
    supplier_manager = SupplierManager()
    supplier = supplier_manager.get_supplier(sku)
...
def get_standard_deviation_of_forecast(sku, cover_period):
...
def get_bias_and_significance(sku, alpha=0.05):
    # Load sales and forecast data
    with open('sales_data.json', 'r') as f:
        sales_data = json.load(f)
    with open('forecast_data.json', 'r') as f:
